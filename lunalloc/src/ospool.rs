...
impl OsPool {
    ...
    pub fn get_next_free_group<CB: PageCommitCB>(&mut self) -> Option<(NonNull<PageGroup>, usize)> {
        ...
        if !self.committed.get(group) {
            ...
            unsafe { self.commit_group::<CB>(group, DEFAULT_COMMIT_PAGES)? };
        }
    ...
    pub unsafe fn get_next_free_page_on_group<CB: PageCommitCB>(
        &mut self,
        group_idx: usize,
        ...
        Some((
            unsafe { group_ptr.cast::<Page>().add(page).cast::<u8>() },
            page,
        ))
    }

    pub fn get_next_free_page<CB: PageCommitCB>(
        &mut self,
    ...
    pub unsafe fn mark_page_full(&mut self, group_idx: usize, page: usize) -> Option<()> {
        if group_idx >= NUM_GROUPS {
            return unsafe {
                self.next?
                    .as_mut()
                    .mark_page_full(group_idx - NUM_GROUPS, page)
            };
        }
    ...
    pub unsafe fn commit_group<CB: PageCommitCB>(
        &mut self,
        group: usize,
        pages: usize,
    ) -> Option<()> {
        if group >= NUM_GROUPS {
            return unsafe {
                self.next?
                    .as_mut()
                    .commit_group::<CB>(group - NUM_GROUPS, pages)
            };
        }
    ...
    pub unsafe fn commit_page<CB: PageCommitCB>(
        &mut self,
        group: usize,
        page: NonZeroUsize,
    ) -> Option<()> {
        if group >= NUM_GROUPS {
            return unsafe {
                self.next?
                    .as_mut()
                    .commit_page::<CB>(group - NUM_GROUPS, page)
            };
        }
        let mut group_ptr = self.group(group)?;
    ...
    pub unsafe fn commit_pages<CB: PageCommitCB>(
        &mut self,
        group: usize,
        page: NonZeroUsize,
        count: NonZeroUsize,
